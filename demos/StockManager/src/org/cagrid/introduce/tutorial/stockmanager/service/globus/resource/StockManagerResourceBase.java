...
import org.cagrid.introduce.tutorial.stockmanager.common.StockManagerConstants;
import org.cagrid.introduce.tutorial.stockmanager.stubs.StockManagerResourceProperties;
 ...
 * 
 */
public abstract class StockManagerResourceBase extends ReflectionResource implements Resource
                                                  {

	static final Log logger = LogFactory.getLog(StockManagerResourceBase.class);

	private StockManagerResourceConfiguration configuration;
    ...
    public StockManagerResourceBase() {
    }
	...
	public gov.nih.nci.cagrid.metadata.ServiceMetadata getServiceMetadata(){
		return ((StockManagerResourceProperties) getResourceBean()).getServiceMetadata();
	}
	
	public void setServiceMetadata(gov.nih.nci.cagrid.metadata.ServiceMetadata serviceMetadata ) throws ResourceException {
        ResourceProperty prop = getResourcePropertySet().get(StockManagerConstants.SERVICEMETADATA);
	...
	public StockManagerResourceConfiguration getConfiguration() {
		...
		try {
			...
			this.configuration = (StockManagerResourceConfiguration) initialContext.lookup(jndiName);
		} catch (Exception e) {
	...
	private void loadServiceMetadataFromFile() {
      if(getServiceMetadata()==null){
		try {
			...
			((StockManagerResourceProperties) this.getResourceBean()).setServiceMetadata((gov.nih.nci.cagrid.metadata.ServiceMetadata) Utils.deserializeDocument(dataFile.getAbsolutePath(),
